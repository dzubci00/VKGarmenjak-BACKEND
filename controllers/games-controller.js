...
const getAllGames = async (req, res, next) => {
  try {
    // Dohvati sve utakmice, sortirane po createdAt (od najstarijih ka najnovijima)
    const games = await Game.find()
      ...
      .sort({ createdAt: 1 }); // Sortiramo po createdAt u uzlaznom redosledu
    ...
    const sortedTournaments = Array.from(tournamentsMap.values()).map(
      (tournament) => ({
        ...
        ), // Sortiraj utakmice u turniru (od najstarijih ka najnovijima)
      })
    );
