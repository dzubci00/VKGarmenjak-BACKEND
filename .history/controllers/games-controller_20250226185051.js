...
const getAllGames = async (req, res, next) => {
  try {
    const games = await Game.find()
      .populate("tournament", "name date") // Dohvati podatke o turniru (samo name i date)
      .populate("playerStats.playerId", "name surname")// Dohvati podatke o igračima (ime prezime)
      .sort({ "tournament.date": 1 }); // Sortiraj prema datumu turnira (1 = rastuće) 
