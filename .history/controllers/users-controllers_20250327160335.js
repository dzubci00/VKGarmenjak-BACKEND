...
const getBirthdays = async (req, res, next) => {
  const today = new Date();
    const month = today.getMonth() + 1; // Mjeseci u JS idu od 0-11
    const day = today.getDate();

    // Pronalazak igrača s istim danom i mjesecom rođenja
    const birthdayPlayers = await Player.find({
      $expr: {
        $and: [
    ...
    console.error("Greška kod dohvaćanja rođendana:", error);
