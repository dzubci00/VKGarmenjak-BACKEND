...
const getMonthlyAttendance = async (req, res, next) => {
  ...
  // Kreiraj prvi i zadnji dan u mjesecu u lokalnoj vremenskoj zoni (samo datum)
  const firstDay = new Date(year, month, 1); // Prvi dan mjeseca
  const lastDay = new Date(year, month + 1, 0); // Posljednji dan mjeseca

  try {
    // Provjeri samo datum bez vremena, jer je moguće da u bazi spremiš samo datum
