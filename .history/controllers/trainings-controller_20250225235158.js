...
const getMonthlyAttendance = async (req, res, next) => {
  const userId = req.userData.userId; // ID igrača za kojeg brojimo dolaske
  const now = new Date();
  
  // Pronađi prvi i zadnji dan tekućeg mjeseca
  // Prvi i zadnji dan u mjesecu u UTC formatu
  const firstDay = new Date(now.getUTCFullYear(), now.getUTCMonth(), 1);
  const lastDay = new Date(now.getUTCFullYear(), now.getUTCMonth() + 1, 0, 23, 59, 59);
  ...
  try {
    const attendanceCount = await Training.countDocuments({
      ...
      "players.playerId": userId // Pravilna provjera unutar players arraya
    });
