...
const updateMembershipStatus = async (req, res, next) => {
  ...
  let user;
  try {
    user = await User.findById(userId).select('name surname membershipPaid membershipType membershipFee refereeMembershipPaid refereeMembershipFee');;
  } catch (err) {
  ...
  res.json({ 
    user: { 
        ...
        membershipPaid, 
        membershipType, 
        refereeMembershipPaid, 
        refereeMembershipFee, 
        membershipFee 
    } 
});
