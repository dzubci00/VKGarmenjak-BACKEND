...
/*const cancelTraining = async (req, res, next) => {
  ...
  try {
    ...
    const playerIndex = training.players.findIndex(player => player.id.toString() === userId);
    if (playerIndex === -1) {
...
const cancelTraining = async (req, res, next) => {
  ...
  try {
    const training = await Training.findByIdAndUpdate(
      trainingId,
      { $pull: { players: { playerId: userId } } }, // Uklanja igrača iz players arraya
...
const getMonthlyAttendance = async (req, res, next) => {
  ...
  try {
    const user=await Training.findById("67be4b8ef2672019f819c99d");
