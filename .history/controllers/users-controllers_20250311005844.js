...
const signup = async (req, res, next) => {
  ...
  try {
    ...
    res.status(201).json({ userId: createdUser.id, email: createdUser.email, token, isAdmin:isAdmin, isVerified:createdUser.isVerified });
  } catch (err) {
    return next(new HttpError('Oops! Something went wrong. Try signing up again later.', 500));
  }
};
...
const resendVerificationEmail = async (req, res, next) => {
  ...
  try {
    ...
    if (!existingUser) {
      return next(new HttpError('Korisnik s tom e-mail adresom ne postoji.', 404));
    }
    ...
    const mailOptions = {
      from: "donat.zubcic@gmail.com",
      to: existingUser.email,
      subject: 'Ponovna verifikacija računa',
      html: `<p>Kliknite na donji link kako biste verificirali svoj račun:</p>
             <a href="${verificationUrl}">${verificationUrl}</a>
             <p>Ovaj link ističe za 1 sat.</p>`
    };

    await transporter.sendMail(mailOptions);

    res.status(200).json({ message: 'Verifikacijski e-mail je ponovno poslan.' });
  } catch (err) {
    return next(new HttpError('Slanje verifikacijskog e-maila nije uspjelo.', 500));
  }
};
