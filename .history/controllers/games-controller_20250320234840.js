...
const getAllGames = async (req, res, next) => {
  try {
    // Dohvati sve utakmice sa podacima o turniru i igračima, sortirane po datumu kreiranja
    const games = await Game.find()
      ...
      .sort({ createdAt: -1 }); // Sortiranje od najnovije ka najstarijoj utakmici
    ...
    games.forEach((game) => {
      ...
      // Dodaj utakmicu u odgovarajući turnir
      tournamentsMap.get(tournamentId).games.push({
        homeTeam: game.homeTeam,
        awayTeam: game.awayTeam,
        score: game.score,
        phase: game.phase,
        createdAt: game.createdAt, // Dodaj createdAt za kasnije sortiranje
    ...
    // Sortiraj utakmice unutar svakog turnira od najnovije ka najstarijoj
    tournamentsMap.forEach((tournament) => {
      tournament.games.sort(
