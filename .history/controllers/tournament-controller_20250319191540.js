...
const addTournamentAndTeams = async (req, res, next) => {
  ...
  try {
    ...
    if (existingTournament) {
      return next(
        new HttpError("Tournament with this name already exists!", 422)
      );
    }
    ...
    const tournament = new Tournament({
      name,
      date,
      teamStandings: teams.map((teamId) => ({ teamId, position: 0 })), // Dodajemo ekipe u turnir
    });
    ...
    res
      .status(201)
      .json({ message: "Tournament added successfully", tournament });
  } catch (err) {
